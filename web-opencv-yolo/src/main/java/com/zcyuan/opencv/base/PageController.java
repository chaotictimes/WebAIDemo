...
public class PageController extends BaseController {
	...
	public void pageOCR(HttpServletResponse response, String imagefile, Integer ocrType) {
		...
		// Mat sourceMat = Imgcodecs.imread(sourcePath, Imgcodecs.CV_LOAD_IMAGE_COLOR);
	...
	public String getPageNoByContours(String filePath) {
		...
		for (int i = 0; i < contours.size(); i++) {
			...
			if (contArea > 1200) {// 此处是根据轮廓面积
				// 红线画出识别的轮廓
				Imgproc.rectangle(image, new Point(r.x, r.y), new Point(r.x + r.width, r.y
	...
	public String getPageNoByTemplate(String filePath) {
		...
		Mat pageimage = Imgcodecs.imread(pagePath, Imgcodecs.CV_LOAD_IMAGE_GRAYSCALE);
	...
	public static void main(String[] args) {
		System.loadLibrary(Core.NATIVE_LIBRARY_NAME);
		Mat markmat1 = Imgcodecs.imread("D:\\test\\abc\\1.png", Imgcodecs.CV_LOAD_IMAGE_GRAYSCALE);
		Mat markmat2 = Imgcodecs.imread("D:\\test\\abc\\2.png", Imgcodecs.CV_LOAD_IMAGE_GRAYSCALE);
	...
	private static MatOfPoint getSimMark(String path) {
		Mat markmat = Imgcodecs.imread(path, Imgcodecs.CV_LOAD_IMAGE_GRAYSCALE);
