package com.zcyuan.opencv.base;
...
import com.zcyuan.opencv.common.utils.CommonUtil;
import com.zcyuan.opencv.common.utils.Constants;
import com.zcyuan.opencv.common.utils.OpenCVUtil;
import com.zcyuan.opencv.common.web.BaseController;
import com.zcyuan.opencv.demo.DemoController;
...
public class BaseMethodController extends BaseController {
	...
	public void binary(HttpServletResponse response, String imagefile, Integer binaryType, Double thresh, Double maxval) {
		...
		// 灰度化
		// Imgproc.cvtColor(source, destination, Imgcodecs.CV_LOAD_IMAGE_GRAYSCALE);
		String sourcePath = Constants.PATH + imagefile;
		logger.info("url==============" + sourcePath);
		// 加载为灰度图显示
		Mat source = Imgcodecs.imread(sourcePath, Imgcodecs.CV_LOAD_IMAGE_GRAYSCALE);
...
//		Imgproc.threshold(source, destination, 0, 255, Imgproc.THRESH_TOZERO_INV);
	...
	public void gaussian(HttpServletResponse response, String imagefile, String kwidth, String kheight, String sigmaX,
		...
		String sourcePath = Constants.PATH + imagefile;
		logger.info("url==============" + sourcePath);
		Mat source = Imgcodecs.imread(sourcePath, Imgcodecs.CV_LOAD_IMAGE_COLOR);
		Mat destination = new Mat(source.rows(), source.cols(), source.type());
...
//		Imgproc.floodFill(source, mask, new Point(0, 0), new Scalar(graysize));
	...
	public void erodingAndDilation(HttpServletResponse response, String imagefile, Double kSize, Integer operateType,
		...
		Mat destination = new Mat(source.rows(), source.cols(), source.type());
		if (isBinary) {
			source = Imgcodecs.imread(sourcePath, Imgcodecs.CV_LOAD_IMAGE_GRAYSCALE);
	...
	public void houghline(HttpServletResponse response, String imagefile, Double threshold1, Double threshold2,
		...
		String sourcePath = Constants.PATH + imagefile;
		logger.info("url==============" + sourcePath);
		Mat source1 = Imgcodecs.imread(sourcePath, Imgcodecs.CV_LOAD_IMAGE_COLOR);// 彩色图
		Mat source2 = Imgcodecs.imread(sourcePath, Imgcodecs.CV_LOAD_IMAGE_GRAYSCALE);// 灰度图
	...
	public void findtemplate(HttpServletResponse response, String imagefile, Integer method, Integer imageType,
		...
		// Mat matchtemp = Imgcodecs.imread(templateimage);
		...
		if (imageType == 0) {// 显示过程图片
			...
			Imgproc.rectangle(source, matchLoc, new Point(matchLoc.x + matchtemp.cols(), matchLoc.y + matchtemp.rows()),
	...
	public void grayHistogram(HttpServletResponse response, String imagefile, Integer cols, Integer imageW,
		...
		for (int i = 1; kedu <= minmaxLoc.maxVal; i++) {
			...
			if (isShow) {
				...
				Imgproc.line(histImg, new Point(0, histImg.height() - 20 - kedu * bin_u),
		...
		for (int i = 0; i < histSize.get(0, 0)[0]; i++) { // 画出每一个灰度级分量的比例，注意OpenCV将Mat最左上角的点作为坐标原点
			...
			Imgproc.putText(histImg, kedu + "", new Point(histImgCols / 256 * kedu, histImgRows - 5), 1, 1, new Scalar(
					255, 0, 0)); // 附上x轴刻度
		}
		try {
	...
	// String sourcePath = Constants.PATH + imagefile;
	// // 加载为灰度图显示
	// Mat imageGray = Imgcodecs.imread(sourcePath, Imgcodecs.CV_LOAD_IMAGE_GRAYSCALE);
